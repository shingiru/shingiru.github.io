 ...
 * 
 * Modified by DS1TZE for the Hangul CW Encoding/Decoding
 */
 ...
 (function () {
    ...
    //--
    COMPLEX_CHOS_HASH = _makeComplexHash(COMPLEX_CHOS);
    MORE_COMPLEX_VOWELS_HASH = (function(array) {
        var length = array.length,
        hash = {},
        code1,
        code2,
        ...
        for (var i = 0; i < length; i++) {
            code1 = array[i][0].charCodeAt(0);
            code2 = array[i][1].charCodeAt(0);
            code3 = array[i][2].charCodeAt(0);
            if (typeof hash[code1] === 'undefined') {
                hash[code1] = {};
            }
            if (typeof hash[code1][code2] === 'undefined') {
                hash[code1][code2] = {};
            }
            hash[code1][code2][code3] = array[i][3].charCodeAt(0);
        }
        return hash;
    })(MORE_COMPLEX_VOWELS);
    //--//
    ...
    //--
    function _isChoJoinable(a, b) {
        return COMPLEX_CHOS_HASH[a] && COMPLEX_CHOS_HASH[a][b] ? COMPLEX_CHOS_HASH[a][b] : false;
    }
    /*function _isJungJoinableEx(a, b, c) {
        return (MORE_COMPLEX_VOWELS_HASH[a] && MORE_COMPLEX_VOWELS_HASH[a][b] && MORE_COMPLEX_VOWELS_HASH[a][b][c]) ? MORE_COMPLEX_VOWELS_HASH[a][b][c] : false;
    }*/
    //--//
                ...
                if (step === 1) {
                    ...
                    if (_isJung(cho)) { // 첫번째 것이 모음이면 1) ㅏ같은 경우이거나 2) ㅙ같은 경우이다
                        ...
                        if (complete_index + step + 1 <= index && _isJung(jung1 = array[complete_index + step + 1].charCodeAt(0))) { //다음것이 있고 모음이면
                            //--
                            /*if (complete_index + step + 2 <= index && _isJung(jung2 = array[complete_index + step + 2].charCodeAt(0))) {
                                result.push(String.fromCharCode(_isJungJoinableEx(cho, jung1, jung2)));
                    ...
                    if (_isCho(jung1)) { //두번째 또 자음이 오면 ㄳ 에서 ㅅ같은 경우이다
                        ...
                        if (_isChoJoinable(cho, jung1)) {
                            cho = _isChoJoinable(cho, jung1);
                    ...
                    if (_isJungJoinable(jung1, jung2)) {
                        ...
                        //--
                        /*if (complete_index + step + 1 <= index && _isJung(jung3 = array[complete_index + step + 1].charCodeAt(0))) {
                            if (_isJungJoinableEx(jung1, jung2, jung3)) {
                                jung1 = _isJungJoinableEx(jung1, jung2, jung3);
                                complete_index++;
                            }
                        } else {
        ...
        for (var i = 0; i < length; i++) {
            ...
            if (stage === 0) { // 초성이 올 차례
                ...
                if (_isJung(code)) { //중성이 오면 문제없음 진행.
                    ...
                    if (_isChoJoinable(previous_code, code) || _isJongJoinable(previous_code, code)) {
                    //--//
                ...
                if (_isJung(code)) { //이번에 중성이면 ㄹ가
                    ...
                    if (_isChoJoinable(previous_previous_code, previous_code)) {
                        stage = 2;
                    } else {
                        _makeHangul(i - 2);
                        stage = 2;
